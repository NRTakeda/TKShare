...
impl TcpServer {
                ...
                r = self.tcp_listener.accept() => {
                    match r {
                        Ok((socket, remote_addr)) => {
                            ...
                            // Same rationale as the outbound side: avoid Nagle's
                            // coalescing delay on the many small protocol frames.
                            if let Err(e) = socket.set_nodelay(true) {
                                warn!("{INNER_NAME}: couldn't set TCP_NODELAY on inbound socket: {e}");
                            }
    ...
    pub async fn connect(&self, ctk: CancellationToken, si: SendInfo) -> Result<(), anyhow::Error> {
        ...
        // Quick Share does a lot of small request/response frames; disabling
        // Nagle's algorithm avoids the ~40ms coalescing delay per round-trip
        // and noticeably improves transfer latency/throughput.
        if let Err(e) = socket.set_nodelay(true) {
            warn!("{INNER_NAME}: couldn't set TCP_NODELAY on outbound socket: {e}");
        }
