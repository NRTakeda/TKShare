...
/// Bytes read from the file and sent per payload chunk. MUST stay at 512 KiB:
/// that's what the official Quick Share uses, and Android rejects larger
/// payload frames and drops the connection (EPIPE / broken pipe) mid-transfer.
/// Throughput gains must come from cheaper per-chunk work, not bigger chunks.
const CHUNK_SIZE: usize = 512 * 1024;
/// Minimum gap between progress (ack_bytes) notifications to the UI. See the
/// inbound side for the rationale: per-chunk notifications flood the channel
/// and re-render the progress bar; ~60ms keeps it smooth with far less
/// overhead. Milestones notify immediately via `update_state`.
const PROGRESS_NOTIFY_INTERVAL: Duration = Duration::from_millis(60);
...
pub struct OutboundRequest {
    ...
    /// Last time a throttled progress notification was sent to the UI.
...
impl OutboundRequest {
    ...
    async fn process_connection_response(
        ...
        let connection_response = v1_frame
            .connection_response
            .as_ref()
            .ok_or_else(|| anyhow!("Peer sent a frame without a connection response"))?;
    ...
    async fn decrypt_and_process_secure_message(
        ...
        let recv_hmac_key = self
            ...
            .recv_hmac_key
            .as_ref()
            .ok_or_else(|| anyhow!("Received a secure message before HMAC key was derived"))?;
        ...
        let key = self
            ...
            .decrypt_key
            .as_ref()
            .ok_or_else(|| anyhow!("Received a secure message before decrypt key was derived"))?;
                ...
                'send_all_files: loop {
                    ...
                    loop {
                        ...
                        let (curr_state, buffer, bytes_read) = {
                            ...
                            // Per-chunk: keep at trace so a big file doesn't
                            // spam thousands of INFO lines (a real throughput hit).
                            trace!("> Currently sending {:?}", curr_state.file_url);
                        ...
                        // NOTE: this runs once per 512 KiB chunk. Keep it cheap:
                        // `trace!` (off by default) and no extra buffer copy —
                        // the previous `sending_buffer` clone was dead weight,
                        // copying up to 512 KiB per chunk just to log its len.
                        trace!(
							...
							v1: Some(location_nearby_connections::V1Frame {
								...
								payload_transfer: Some(PayloadTransferFrame {
									...
									payload_chunk: Some(PayloadChunk {
										...
										flags: Some(0),
										// Move the read buffer in (trimmed to what was
										// actually read) instead of allocating+copying
										// a second Vec of the whole chunk.
										body: Some({ let mut b = buffer; b.truncate(bytes_read); b }),
									}),
                        ...
                        // Throttled: progress fires many times per second.
                        self.update_state_progress(|e| {
    ...
    async fn finalize_key_exchange(
        ...
        // Reject a peer-supplied point that isn't actually on the P-256 curve
        // instead of panicking (see the inbound side for details).
        let peer_key = Option::<PublicKey>::from(PublicKey::from_encoded_point(&encoded_point))
            .ok_or_else(|| anyhow!("Peer sent an invalid P-256 public key (not on curve)"))?;
    ...
    async fn encrypt_and_send(&mut self, frame: &OfflineFrame) -> Result<(), anyhow::Error> {
        ...
        // Serialize the header+body once and reuse it for both the HMAC and the
        // SecureMessage. The previous code called `hb.encode_to_vec()` twice,
        // serializing the full (chunk-sized) encrypted body an extra time on
        // every single chunk.
        let hb_bytes = hb.encode_to_vec();

        let mut hmac = HmacSha256::new_from_slice(self.state.send_hmac_key.as_ref().unwrap())?;
        hmac.update(&hb_bytes);
    ...
    async fn send_frame(&mut self, data: Vec<u8>) -> Result<(), anyhow::Error> {
        ...
        // Write the 4-byte length prefix and the body separately instead of
        // copying the whole (chunk-sized) `data` into a new prefixed buffer.
        // TCP_NODELAY is on, so this still goes out promptly without Nagle
        // coalescing delay.
    ...
    /// Like `update_state`, but for high-frequency progress updates: the state
    /// mutation always applies, while the UI notification is throttled to at
    /// most one every `PROGRESS_NOTIFY_INTERVAL`. Avoids flooding the channel
    /// with a message per chunk on fast transfers.
    async fn update_state_progress<F>(&mut self, f: F)
    where
        F: FnOnce(&mut InnerState),
    {
        f(&mut self.state);
        ...
        let now = Instant::now();
        let should_notify = match self.last_progress_notify {
