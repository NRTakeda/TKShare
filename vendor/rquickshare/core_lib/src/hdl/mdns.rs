...
/// How often to re-announce the mDNS service while visible, so an Android
/// device that started scanning after us (or over a flaky link) still finds
/// the PC quickly. Kept short but not chatty.
const VISIBLE_RESEND_INTERVAL: Duration = Duration::from_secs(10);
...
impl MDnsServer {
    ...
    pub async fn run(&mut self, ctk: CancellationToken) -> Result<(), anyhow::Error> {
        ...
        let mut resend_interval =
            interval_at(Instant::now() + VISIBLE_RESEND_INTERVAL, VISIBLE_RESEND_INTERVAL);
        ...
        loop {
            tokio::select! {
                ...
                _ = resend_interval.tick() => {
                    // While visible, periodically re-announce the service. The
                    // BLE-triggered resend only fires when an Android device is
                    // actively scanning over BLE; if BLE is unavailable, or the
                    // phone started discovery after us, the PC could otherwise
                    // go unnoticed. A cheap periodic resend keeps it findable.
                    if visibility == Visibility::Visible || visibility == Visibility::Temporarily {
                        if let Err(e) =
                            self.daemon.register_resend(self.service_info.get_fullname())
                        {
                            warn!("{INNER_NAME}: periodic mDNS resend failed: {e}");
                        }
                    }
                }
                _ = interval.tick() => {
