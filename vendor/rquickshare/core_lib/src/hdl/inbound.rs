...
/// Minimum gap between progress (ack_bytes) notifications to the UI. Per-chunk
/// notifications flood the channel and re-render the progress bar hundreds of
/// times a second on fast transfers; ~60ms (≈16fps) keeps the bar smooth with
/// a fraction of the overhead. Milestones (state changes, completion) bypass
/// this and notify immediately.
...
impl InboundRequest {
    ...
    async fn process_ukey2_client_finish(
        ...
        // The commitment is set when the earlier ClientInit was processed; a
        // peer that jumps straight to ClientFinish would leave it unset.
        let cipher_commitment = self
            .state
            .cipher_commitment
            .as_ref()
            .ok_or_else(|| anyhow!("UKey2: ClientFinish received before ClientInit commitment"))?;
    ...
    async fn decrypt_and_process_secure_message(
        ...
        // A peer could send a secure message before the key exchange has
        // derived these keys (out-of-order frame); treat that as an error
            ...
            location_nearby_connections::v1_frame::FrameType::PayloadTransfer => {
                ...
                match header.r#type() {
                    payload_header::PayloadType::Bytes => {
                        // Per-chunk: trace to avoid log spam throttling throughput.
                    ...
                    payload_header::PayloadType::File => {
                        // Per-chunk: trace, not info (a big file is thousands of
                        // chunks, and INFO logging per chunk throttles throughput).
    ...
    async fn finalize_key_exchange(
        ...
        // The peer-supplied point may be well-formed but not actually on the
        // P-256 curve. `from_encoded_point` returns a constant-time `CtOption`;
        // rejecting it here turns a malicious/garbage key into a clean error
        // instead of a panic (and avoids an invalid-curve footgun).
    ...
    /// most one every `PROGRESS_NOTIFY_INTERVAL`. This avoids flooding the
    /// channel (and the progress bar) with a message per chunk on fast links.
